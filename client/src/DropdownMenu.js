...
function DropdownMenu({ returnURL, isEmployer }) {
    ...
    if (isEmployer) {
        return (
            <Menu>
                ...
                <MenuList>
                    ...
                    {isAuthenticated && <MenuItem onClick={() => navigate("/employer/jobs")}>Saved Jobs</MenuItem>}
        ...
        return (
            <Menu>
                <MenuButton as={IconButton} aria-label='Options' icon={<ChevronDownIcon />} variant='outline' color='white' />
                <MenuList>
                    {isAuthenticated && <MenuItem onClick={() => navigate("/candidate")}>Profile</MenuItem>}
